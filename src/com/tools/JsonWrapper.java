 ...
 * from copy
 * 
 * @packageName com.niuyouba.tools
...
public class JsonWrapper {
	...
	public JsonWrapper(String jsonStr) {
		this.json = JSONObject.fromObject(StringUtils.isBlank(jsonStr) ? "{}"
				: jsonStr);
	}
	...
	private String getJsonString(String key) {
		...
		try {
			...
			JarLogTools.jarLog.error("getJsonString error", e);
		}
		return value;
	}
	...
	public List<?> getList(int key) {
		...
		try {
			...
			JarLogTools.jarLog.error("getList error", e);
		}
	...
	public List<?> getList(int key, Class clazz) {
		...
		try {
			...
			for (int index = 0; index < arr.size(); index++) {
				...
				result.add(JSONObject.toBean(jsonObj, clazz));
			}
	...
	public Map<String, ?> getMap(int key) {
		...
		try {
			...
			JarLogTools.jarLog.error("getMap error", e);
		}
	...
	public Map<String, ?> getMap(int key, Class clazz) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				result.put(strkey, JSONObject.toBean(
						(JSONObject) value.get(strkey), clazz));
			}
		} catch (Exception e) {
			JarLogTools.jarLog.error("getMap error", e);
		}
